...
typedef float32x4_t uir_f32x4;
typedef int32x4_t uir_i32x4;
...
UIR_INLINE uir_f32x4 uir_i16x8_lerp(uir_i16x8 a, uir_i16x8 b, uir_i16x8 fraction) {
    uir_i16x8 diff = vsubq_s16(b, a);
    return vqrdmlahq_s16(a, diff, fraction); 
}

UIR_INLINE uir_f32x4 uir_i32x4_load_unaligned(const int* data) {
    return vld1q_s32((int*)data);
}

UIR_INLINE uir_f32x4 uir_f32x4_load_unaligned(const float* data) {
    return vld1q_f32((float*)data);
}

UIR_INLINE uir_f32x4 uir_f32x4_new_splat(float value) {
    return vdupq_n_f32(value);
}

UIR_INLINE uir_f32x4 uir_f32x4_sub(uir_f32x4 a, uir_f32x4 b) {
    return vsubq_f32(a, b);
}

UIR_INLINE uir_f32x4 uir_f32x4_add(uir_f32x4 a, uir_f32x4 b) {
    return vaddq_f32(a, b);
}

UIR_INLINE uir_f32x4 uir_f32x4_mul(uir_f32x4 a, uir_f32x4 b) {
    return vmulq_f32(a, b);
}

UIR_INLINE uir_f32x4 uir_f32x4_floor(uir_f32x4 a) {
    return vrndq_f32(a);
}

UIR_INLINE uir_i32x4 uir_f32x4o_i32x4(uir_f32x4 a) {
    return vcvtq_s32_f32(a);
}

UIR_INLINE uir_i16x8 uir_f32x4o_i16x8(uir_f32x4 a) {
    return vcvtq_f32_s32(a);
} 

UIR_INLINE uir_f32x4 uir_i32x4o_f32x4(uir_i32x4 a) {
    return vcvtq_s32_f32(a);
}

UIR_INLINE uir_i16x8 uir_i16x8_splat_1111_1111(uir_i16x8 a) {
    return __builtin_shufflevector(a, a, 1,1,1,1,1,1,1,1); 
}

UIR_INLINE uir_i16x8 uir_i16x8_splat_3333_3333(uir_i16x8 a) {
    return __builtin_shufflevector(a, a, 3,3,3,3,3,3,3,3); 
}
...
typedef __m128 uir_f32x4;
typedef __m128i uir_i32x4;
...
UIR_INLINE uir_i32x4 uir_i32x4_load_unaligned(const int* data) {
    return _mm_loadu_si128((__m128i*)data);
}
...
UIR_INLINE uir_i32x4 uir_f32x4o_i16x8(uir_f32x4 a) {
    return _mm_cvttps_epi32(a);
}

UIR_INLINE uir_i16x8 uir_i16x8_splat_1111_1111(uir_i16x8 a) {
    return _mm_shuffle_epi32(_mm_shufflelo_epi16(a, 85), 0);
}

UIR_INLINE uir_i16x8 uir_i16x8_splat_3333_3333(uir_i16x8 a) {
    return _mm_shuffle_epi32(_mm_shufflelo_epi16(a, 255), 0);
}
...
UIR_INLINE void uir_internal_raster(
    int16_t* output, 
    ...
    const int16_t* bottom_colors_a) 
{
    ...
    uir_f32x4 x0y0x1y1_l = uir_f32x4_load_unaligned(coords);
    uir_f32x4 tile_offsets = uir_f32x4_load_unaligned(tile_info->offsets);

    // Adjust for tile offset and center pixels at 0.5
    uir_f32x4 x0y0x1y1_adjust = uir_f32x4_add(uir_f32x4_sub(x0y0x1y1_l, tile_offsets), uir_f32x4_new_splat(0.5f));
    uir_f32x4 x0y0x1y1 = uir_f32x4_floor(x0y0x1y1_adjust);

    uir_i32x4 texture_sizes = uir_i32x4_load_unaligned(texture_sizes_a);
    uir_f32x4 texture_sizes_f = uir_i32x4o_f32x4(texture_sizes); 

    // TODO: Bool check
    { 
        uir_f32x4 uv0uv1 = uir_f32x4_load_unaligned(uv_coords); 
        uir_f32x4 uv2uv3 = uir_f32x4_load_unaligned(uv_coords + 4);
        //
        ...
        uir_f32x4 uv_fraction_f = uir_f32x4_mul(uir_f32x4_sub(x0y0x1y1_adjust, x0y0x1y1), uir_f32x4_new_splat(0x7fff));
        uir_i16x8 uv_fraction_i = uir_i16x8_sub(v_7fff, uir_f32x4_i16x8(uv_fraction_f))
        
        fixed_u_fraction = uir_i16x8_splat_1111_1111(uv_fraction_i));
        fixed_v_fraction = uir_i16x8_splat_3333_3333(uv_fraction_i));

        uir_i32x4 uv0uv1_i = uir_f32x4o_i32x4(uv0uv1);
    ...
    uir_i32x4 x0y0x1y1_i = uir_f32x4o_i32x4(x0y0x1y1);
...
void uir_render_aligned_texture(
    int16_t* output, 
    const uir_TileInfo* tile_info,
    const int16_t* texture_data, 
    const int* texture_sizes_a,
    ...
    const float* uv_coords)
{
    uir_internal_raster(output, tile_info, texture_data, texture_sizes_a, coords, uv_coords, NULL, NULL);
}
